...
class DistributeurSchema(PlainDistributeurSchema):
    livraison = fields.Nested(PlainLivraisonSchema(), dump_only=True)
    
class CommandeSchema(PlainCommandeSchema):
    livraison = fields.Nested(PlainLivraisonSchema(), dump_only=True)
    produit = fields.List(fields.Nested(PlainProduitSchema()), dump_only=True)

class DepotSchema(PlainDepotSchema):
    client_id = fields.Nested(PlainClientSchema(),load_only=True)
